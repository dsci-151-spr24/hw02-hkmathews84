...
***We typically choose the median over mean because a mean can be skewed by a few outliers/extremes. Whereas a median is not skewed, better for datasets that may have outliers.***
...
***The summary statistic of mean of the median data most accurately describes the distribution.***
...
```{r}
college_recent_grads %>%
  select(major, median, p25th, p75th, major_type) %>%
  arrange(desc(median)) %>%
  filter(
    major_type == "stem",
    median <= 36000)
...
```{r}
college_recent_grads %>%
  ggplot(aes(x = median, y = women, color = major_type)) +
  geom_point()
```

***How these three variables are associated: STEM fields are considered male-dominant fields and tend to be the higher earning fields, so this scatterplot shows the relation between STEM and non-STEM fields that women choose and their median income.***
